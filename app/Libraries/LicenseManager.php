...
class LicenseManager
{
    ...
    public function __construct()
    {
        ...
        // Gunakan WRITEPATH untuk keamanan
        $this->licenseFile  = WRITEPATH . 'cache/._lhc';
        $this->lockdownFile = WRITEPATH . 'cache/._lckdwn';
    }

    // --- SEMUA FUNGSI PRIVATE ANDA DARI HnLbs_Mgr ADA DI SINI ---
    // (Tidak perlu diubah, kecuali path file)
    ...
    public function iall(): bool { $d = $this->_get_ll_d(); if ($d === null || !isset($d->st) || $d->st !== 'VALID') { return false; } if (isset($d->ex_dt) && $d->ex_dt !== null && strtotime($d->ex_dt) < time()) { return false; } return true; }
    public function cfi(): bool { return true; /* Implementasi hash check file jika diperlukan */ }
    public function rv_p(): bool|string { $d = $this->_get_ll_d(); if ($d === null || !isset($d->lk)) return true; $lckd = $d->lckd ?? 0; $sec = 3600; /* Re-validasi setiap 1 jam */ if ((time() - $lckd) > $sec) { $sr = $this->_v_l_s($d->lk); if (isset($sr->status) && $sr->status === 'VALID') { $this->_s_l_d($d->lk, $sr); return true; } else { return $sr->message ?? 'Gagal memverifikasi lisensi dengan server.'; } } return true; }
